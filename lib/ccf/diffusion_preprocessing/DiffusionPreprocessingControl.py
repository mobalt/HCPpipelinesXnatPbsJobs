...
import ccf.diffusion_preprocessing.SubmitDiffusionPreprocessingBatch as SubmitDiffusionPreprocessingBatch
import ccf.diffusion_preprocessing.one_subject_completion_xnat_checker as one_subject_completion_xnat_checker
...
class ControlPanelWidget(QWidget):
	...
	def on_launch_click(self):
		...
		if len(launch_subject_list) < 1:
			...
			if self._login.exec_() == QDialog.Accepted:
				SubmitDiffusionPreprocessingBatch.do_submissions(
	...
	def build_status_list(self, subject_list):
		...
		for subject in self._subject_list:
			prereqs_met = self.prereq_checker.are_prereqs_met(self.archive, subject)
			resource = self.archive.diffusion_preproc_dir_name(subject)
			resource_exists = self.completion_checker.does_processed_resource_exist(archive, subject)

			if resource_exists:
				resource_fullpath = self.archive.diffusion_preproc_dir_full_path(subject)
	...
	# def submitJob(self, username, password, project, subject_id, classifier, clean_output_first, processing_stage, walltime_limit_hrs, vmem_limit_gbs, output_resource_suffix, brain_size, use_prescan_normalized):
		# result = subprocess.call([os.environ["XNAT_PBS_JOBS"] + "/lib/ccf/diffusion_preprocessing/submit_job.py", username, password, project, subject_id, classifier, 
...
class MyMainWindow(QMainWindow):
	...
	def __init__(self, archive, subject_list, prereq_checker, completion_checker, run_status_checker):
		super().__init__()
		self.title = "Diffusion Preprocessing Control"
